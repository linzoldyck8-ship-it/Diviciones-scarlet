...
# Asegurar tabla admin_config con Primary Key en Supabase (soluciona el error visual)
if engine is not None and tb_admin_config:
    inspector = inspect(engine)
    if not inspector.has_table(tb_admin_config):
        df_init_admin = pd.DataFrame([{"clave": "password_admin", "valor": "admin123"}])
        df_init_admin.to_sql(tb_admin_config, engine, if_exists='replace', index=False)
    # Ejecutar alter table seguro para garantizar la PK si ya existe
    try:
        with engine.begin() as conn:
            conn.execute(f"CREATE TABLE IF NOT EXISTS {tb_admin_config} (id SERIAL PRIMARY KEY, clave TEXT UNIQUE NOT NULL, valor TEXT NOT NULL);")
    except Exception:
        pass
